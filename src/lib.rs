...
// Only docs.rs passes `--cfg docsrs`, and only it builds on nightly, so this
// is inert everywhere else. It is what puts the "available on crate feature X
// only" badges on gated items. `auto_cfg` is on by default under this gate, so
// individual items do not need annotating.
...
pub use crate::{
    ...
    db::{
        ...
        GetIntoBufferResult, LiveFile, LiveWalFile, MultiThreaded, PrefixProber, Range,
        SingleThreaded, ThreadMode, WalFileType,
    },
    db_iterator::{
...
/// The raw `librocksdb` bindings this crate is built against.
///
/// [`AsRawPtr`] hands out pointers to types from this crate, such as
/// `ffi::rocksdb_t`, so callers need a way to name them. Reaching them through
/// a separate `rust-librocksdb-sys` dependency would mean keeping that
/// version in lockstep with this crate's by hand, and every sys bump would
/// break it. Going through this re-export keeps the two tied together.
///
/// Everything here is generated by bindgen and is not covered by this crate's
/// semver guarantee. It changes whenever the vendored RocksDB does.
#[cfg(feature = "raw-ptr")]
pub use rust_librocksdb_sys as ffi_raw;
